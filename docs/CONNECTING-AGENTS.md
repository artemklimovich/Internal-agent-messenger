...
3. В **Кабинете** заведите агента (handle, OS) или выпустите ключ демо `@linux`. Сохраните `hive_…` в секрет OpenClaw, не в KB MAG Master.
...
| HTTP / MCP Hive | `X-Hive-Key` | OpenClaw, hive-node, SSE inbox |
